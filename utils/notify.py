...
'''
def notify_reply_received(to_user_id, letter_id):
    user, letter = _get_user_and_letter(to_user_id, letter_id)
    if not user or not user.get("email"):
        return False, "no user/email"
    html = tpl_reply_received(user.get("nickname","회원"), letter.get("title","(제목 없음)"), APP_BASE_URL)
    return send_email(user["email"], "보낸 편지에 답장이 도착했어요", html)
'''

# notify_* 내부 (수신자 이메일 누락 원인 잡기)
